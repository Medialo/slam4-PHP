...
class ControllerTrajet {
    ...
    public static function read() {
        ...
        //$v = ModelTrajet::gettrajetByImmat($i); //appel au mod�le pour gerer la BD
        $v = ModelTrajet::select($i); 
        if($v != null){
            $controller='trajet';
            $view='detail';
            $pagetitle="D�tail du trajet";
            require (File::build_path(["view",'view.php']));
        }
...
//        require (File::build_path(["view","trajet",'update.html']));
    ...
    public static function created() {
        ...
        $tab_v = ModelTrajet::getAlltrajets(); //appel au mod�le pour gerer la BD
        $controller='trajet';
        $view='created';
        $pagetitle="trajet cr��! + Liste";
        require (File::build_path(["view",'view.php']));
        // require ('../view/trajet/formulairetrajet.html'); //"redirige" vers la vue
    }
    ...
    public static function delete()
    {
        ...
        if(isset($_GET['id'])){
            ...
            $tab_v = ModelTrajet::selectAll(); //appel au mod�le pour gerer la BD
            $controller='trajet';
            $view='deleted';
